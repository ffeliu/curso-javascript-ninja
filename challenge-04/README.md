...
*/
carro.adicionarPessoas = function (qtdePessoa) {

    if(qtdePessoa < 0) {
        carro.quantidadePessoa += qtdePessoa;

        carro.quantidadePessoa < 0 ? carro.quantidadePessoa = 0 : carro.quantidadePessoa = carro.quantidadePessoa;
    }
    else{
        var vagas = carro.assentos - carro.quantidadePessoa;

        if( carro.quantidadePessoa >= carro.assentos) {
            return "O carro já está lotado";
        }
        else if( qtdePessoa > vagas) {
            if( vagas === 1)
    ...
    if(carro.quantidadePessoa === 1)
        return "Já temos " + carro.quantidadePessoa + " pessoa no carro!";
    else
